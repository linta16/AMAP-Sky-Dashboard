    ...
    for i in range(start, len(html)):
        if html[i] == '[':
            ...
            if depth == 0:
                ...
                break
    if end == -1:
        print(f"WARNING: Could not find end of {varname}!")
        return html
    result = html[:idx] + f'var {varname}={new_json};' + html[end:]
    print(f"  Replaced {varname}: {len(html)} -> {len(result)} bytes")
    return result
...
def replace_js_str_var(html, varname, new_value):
    ...
    return html[:idx] + f"var {varname}='{new_value}';" + html[end+2:]
