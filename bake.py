...
# Bake defects - match var D=ANY_ARRAY;
d_json = json.dumps(defects, ensure_ascii=False)
html, n = re.subn(r'var D=\[[\s\S]*?\];', f'var D={d_json};', html)
print(f"Defects baked: {n} replacement(s)")

# Bake QA - match var _bakedQA=ANY_ARRAY;
qa_json = json.dumps(qa_rows, ensure_ascii=False)
qa_ts = f'Auto-refreshed: {stamp} - {len(qa_rows)} QA records'
html, n = re.subn(r'var _bakedQA=\[[\s\S]*?\];', f'var _bakedQA={qa_json};', html)
print(f"QA baked: {n} replacement(s)")

# Bake QA timestamp
html, n = re.subn(r'var _bakedQAts=.*?;', f"var _bakedQAts='{qa_ts}';", html)
print(f"QA ts baked: {n} replacement(s)")
